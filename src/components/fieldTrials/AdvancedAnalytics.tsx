...
// If recharts is not installed, fallback to a simple SVG below
let RechartsLineChart, XAxis, YAxis, Tooltip, CartesianGrid, Line, ResponsiveContainer, Bar;
try {
  // @ts-ignore
  ({ LineChart: RechartsLineChart, XAxis, YAxis, Tooltip, CartesianGrid, Line, ResponsiveContainer, Bar } = require('recharts'));
} catch {}
...
const AdvancedAnalytics: React.FC = () => {
  ...
  useEffect(() => {
    ...
    getFieldVisitAnalytics()
      .then((data) => {
        // Convert monthlyTrends object to sorted array
        const arr = Object.entries(data.monthlyTrends || {})
          .map(([month, count]) => ({ month, count: Number(count) }))
          .sort((a, b) => a.month.localeCompare(b.month));
        setMonthlyVisits(arr);
        setLoadingVisits(false);
      })
      .catch((e) => {
        setErrorVisits(e.message || 'Error loading field visit trends');
        setLoadingVisits(false);
      });
  }, []);
        ...
        <TabsContent value="charts" className="space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              ...
              <CardContent>
                {loadingVisits ? (
                  <div className="h-64 flex items-center justify-center text-gray-500">Loading monthly field visits...</div>
                ) : errorVisits ? (
                  <div className="h-64 flex items-center justify-center text-red-500">{errorVisits}</div>
                ) : monthlyVisits.length === 0 ? (
                  <div className="h-64 flex items-center justify-center text-gray-500">No field visit data</div>
                ) : RechartsLineChart ? (
                  <ResponsiveContainer width="100%" height={240}>
                    <RechartsLineChart data={monthlyVisits} margin={{ top: 16, right: 24, left: 8, bottom: 8 }}>
                  ...
                  <svg width="100%" height="240" viewBox="0 0 400 240">
                    {monthlyVisits.map((d, i) => {
                      const max = Math.max(...monthlyVisits.map(m => m.count));
                      const barHeight = (d.count / max) * 180;
                      return (
                        <g key={d.month}>
                          <rect x={i * 40 + 30} y={220 - barHeight} width="30" height={barHeight} fill="#16a34a" />
                          <text x={i * 40 + 45} y={235} fontSize="12" textAnchor="middle">{d.month}</text>
                          <text x={i * 40 + 45} y={220 - barHeight - 5} fontSize="12" textAnchor="middle">{d.count}</text>
                        </g>
                      );
                    })}
                    <line x1="20" y1="220" x2="380" y2="220" stroke="#ccc" />
                  </svg>
                )}
              </CardContent>
            </Card>
