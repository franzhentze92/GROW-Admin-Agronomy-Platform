...
const EnterAnalysisPage: React.FC = () => {
    ...
    }, [analysisTasks, selectedConsultant, selectedAnalysisType, selectedCategory, selectedStatuses, selectedMonth, sortConfig]);
    ...
    const ganttTasks: GanttTask[] = useMemo(() => {
        return displayedAnalyses
            .map(task => {
                ...
                return {
                    id: task.id,
                    name: `${task.client_name} - ${task.category} (${task.crop})`,
            ...
            <Card>
                ...
                <CardContent className="flex flex-col sm:flex-row gap-4">
                     ...
                     <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Button variant={selectedCategory === 'all' ? 'default' : 'outline'} size="sm" onClick={() => setSelectedCategory('all')}>All Categories</Button>
                                ...
                                {displayedAnalyses.map((analysis) => {
                                    ...
                                    return (
                                        <tr key={analysis.id} className="border-b hover:bg-muted/50">
                                            ...
                                            <td className="cursor-pointer" onClick={() => fullAnalysis && handleViewAnalysis(fullAnalysis)}>
                                                {fullAnalysis?.unit_price ? `$${fullAnalysis.unit_price.toFixed(2)}` : '-'}
                                            </td>
                                            <td className="cursor-pointer" onClick={() => fullAnalysis && handleViewAnalysis(fullAnalysis)}>
                                                {fullAnalysis?.total_price ? `$${fullAnalysis.total_price.toFixed(2)}` : '-'}
                                            </td>
                                            <td className="cursor-pointer" onClick={() => fullAnalysis && handleViewAnalysis(fullAnalysis)}>
