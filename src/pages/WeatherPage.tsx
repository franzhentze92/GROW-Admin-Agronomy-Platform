  ...
  const fetchWeatherData = async () => {
    ...
    try {
      ...
      for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
        ...
        const temperature = 20 + (dayIndex % 7); // 20-26°C
        const rain = [0, 2.5, 8, 0, 1.5, 4, 0][dayIndex % 7]; // 0-8mm
        const wind = 3 + (dayIndex % 5); // 3-7 m/s
        const humidity = 55 + (dayIndex % 6) * 5; // 55-85%
        const max_temp = temperature + 3;
        const min_temp = temperature - 4;
        mockWeatherDays.push({
