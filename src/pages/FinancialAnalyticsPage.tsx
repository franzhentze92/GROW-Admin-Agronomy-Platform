...
const FinancialAnalyticsPage: React.FC = () => {
  ...
  // Helper to filter costs by selectedCategory and timeRange
  const filterCosts = (costs: typeof costs) => {
    let filtered = costs;
    // Filter by category
  ...
  // Calculate expense metrics from filtered data
  const expenseMetrics = useMemo(() => {
    const filteredCosts = filterCosts(costs);
    const totalExpenses = filteredCosts.reduce((sum, cost) => sum + cost.amount, 0);
    const monthlyExpenses = filteredCosts.filter(cost => cost.expense_type === 'monthly').reduce((sum, cost) => sum + cost.amount, 0);
    const oneTimeExpenses = filteredCosts.filter(cost => cost.expense_type === 'one_time').reduce((sum, cost) => sum + cost.amount, 0);
    ...
    const monthlyExpenseData = filteredCosts.reduce((acc, cost) => {
      const month = new Date(cost.date).toLocaleDateString('en-US', { month: 'short' });
      acc[month] = (acc[month] || 0) + cost.amount;
      return acc;
    }, {} as Record<string, number>);

    // Sort months by actual date (oldest to newest)
    const monthOrder = Object.keys(monthlyExpenseData)
      .map(m => {
        // Try to find the first date in costs for this month
        const dateObj = filteredCosts.find(c => new Date(c.date).toLocaleDateString('en-US', { month: 'short' }) === m);
        return { m, d: dateObj ? new Date(dateObj.date) : new Date('2100-01-01') };
      })
    ...
    const categoryPerformance = Object.entries(costsByCategory).map(([category, amount]) => ({
      ...
      count: filteredCosts.filter(cost => cost.category === category).length,
    }));

    // Recent expenses (last 30 days)
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
    const recentExpenses = filteredCosts.filter(cost => new Date(cost.date) >= thirtyDaysAgo);
    ...
    const expenseTypeBreakdown = filteredCosts.reduce((acc, cost) => {
  ...
  }, [costs, selectedCategory, timeRange]);

  if (!hasRole(['admin', 'super-admin'])) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-xl font-semibold text-red-600">You are not authorized to view this page.</div>
      </div>
    );
  }
