...
const AdminAddEventPage: React.FC = () => {
  const [event, setEvent] = useState<Partial<Event>>(initialEvent);
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setEvent(prev => ({ ...prev, image: publicUrlData.publicUrl }));
  ...
  return (
    <div className="w-full flex justify-center bg-gray-50 min-h-screen">
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow p-10 my-10 mx-4">
        <h1 className="text-2xl font-bold mb-6">Add New Event (Super Admin)</h1>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={loading} className="bg-lime-600 text-white px-6 py-2 rounded font-semibold hover:bg-lime-700 transition">
